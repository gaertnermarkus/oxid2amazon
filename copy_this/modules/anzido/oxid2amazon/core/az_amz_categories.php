...
class az_amz_categories extends oxbase
{
    ...
    protected function _SaveCategoryMapping($soxId, $aThemeData)
    {
        ...
        foreach ($aThemeData as $aCat) {
            if($aCat['CATID']) {
                $iCounter++;
                $sInsert = "INSERT INTO " . $sTable . " SET OXID=" . $oDB->quote(
                        oxUtilsObject::getInstance()->generateUID()
                    ) . ", OXOBJECTID=" . $oDB->quote($soxId) . ", OXTYPE='oxcategory', OXSORT=" . $oDB->quote(
                        $iCounter
                    ) . ", D3AMAZONCATID=" . $oDB->quote($aCat['CATID']);
                #echo "<br>" . $sInsert;
                $oDB->execute($sInsert);
            }
            $sInsert = "";
    ...
    public function getAmazonCategories4Category($soxId)
    {
        $oDB     = oxDb::getDb(TRUE);
        $sTable  = getViewName($this->_sCoreTable);
        $sSelect = "SELECT * FROM " . $sTable . " WHERE OXOBJECTID=" . $oDB->quote(
                $soxId
            ) . " and OXTYPE='oxcategory' ORDER BY OXSORT DESC";
    ...
    public function getDefaultCategory($oArticle)
    {
        ...
        $sQ .= "where $sO2CView.oxobjectid=" . $oDB->quote(
                $oArticle->getId()
            ) . " and $sCatView.oxactive" . (($sLang) ? "_$sLang" : "") . " = 1 order by $sO2CView.oxtime limit 0,1";
    ...
    public function getAmazonCategoriesFromCSV()
    {
        ...
        $sPath          = getShopBasePath() . "/modules/d3/d33oxid2amazon/modules/amazon/themes/" . $sThemeFile;
        #echo $sPath;
