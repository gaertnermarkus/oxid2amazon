...
class az_amz_Config
{
    ...
    public function __construct($sShopId = null)
    {
        ...
        $sModuleDir = $oModule->getModulePath('oxid2amazon');
        $sModulePath = "modules/" . $sModuleDir . "/modules/az_amazon_config.inc.php";
        
        $sModulePath = oxConfig::getInstance()->getConfigParam( 'sShopDir' ).$sModulePath;
    ...
    public function loadFromDb()
    {
        ...
        $rsResult = $db->Execute(
                "SELECT az_varname, az_varvalue FROM az_amz_config WHERE oxshopid=?", array($this->_sShopId)
        );
    ...
    public function saveToDatabase()
    {
        ...
        foreach (array_keys($this->_aNewKeys) as $key)
        {
            ...
            if (false === $db->GetOne($sExistsQ, array($key)))
